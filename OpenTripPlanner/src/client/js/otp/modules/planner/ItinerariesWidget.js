...
otp.widgets.ItinerariesWidget =
    ...
    updateItineraries : function(
        ...
        if (append) {
            for (i = 0; i < itineraries.length; i++)
                this.itineraries.push(itineraries[i]);
            if (this.itineraries && this.itineraries[this.itineraries.length-1].tripPlan.queryParams.sorted) {
                this.itineraries.sort((i1, i2) => i1.itinData.endTime - i2.itinData.endTime);
                var c = 1;
            }
        ...
        this.itinsAccord.accordion({
            ...
            // Per espandere il contenuto dell'accordion
            heightStyle: "content"
        });
    ...
    renderItinerary : function(itin, index, alerts) {
        ...
        for(var l=0; l<itin.itinData.legs.length; l++) {
            ...
            $(legDiv).accordion({
                header : 'h3',
                // Collasso tutti tratti, non solo quelli transit
        ...
        .append('<div class="otp-itinTripSummaryLabel">' + _tr("Travel by") + '</div><div class="otp-itinTripSummaryText">'+otp.config.modes[itin.tripPlan.queryParams.mode]+'</div>')
        ...
        var transferTime = itin.getTransferTime();
        if (transferTime !== "")
            tripSummary.append('<div class="otp-itinTripSummaryLabel">' + _tr("Total_transfer_time") + '</div><div class="otp-itinTripSummaryText">'+itin.getTransferTime()+'</div>');
