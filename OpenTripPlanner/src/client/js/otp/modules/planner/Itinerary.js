...
otp.modules.planner.Itinerary = otp.Class({
    ...
    /* Tempo totale impiegato per passare da un mezzo al successivo,
    include i tempi di spostamento fra le fermate e le attese */
    getTransferTime: function() {
        let transferEnd;
        let transferStart;
        var transferTime = 0;
        // evito access, egress e il primo transit
        for(var l=1; l<this.itinData.legs.length -1; l++) {
            if(!this.itinData.legs[l].transitLeg) {
                transferStart = this.itinData.legs[l].startTime;
                transferEnd = this.itinData.legs[l + 1].startTime;
            } else {
                transferStart = this.itinData.legs[l].endTime;
                transferEnd = this.itinData.legs[l + 1].startTime;
            }
        ...
        return otp.util.Time.secsToHrMinSec( transferTime/1000.0 );
    },
