...
otp.widgets.tripoptions.LocationsSelector =
    ...
    doAfterLayout : function() {
        ...
        $("#"+this.id+"-start").change($.proxy(function() {
            const val = $("#" + this.id + "-start").val().trim();
            if (!this.checkCoordinates(val))
                alert("Coordinate non valide");
            else
                this.tripWidget.module.setStartPoint(this.extractLatLng(val));
        }, this));
        ...
        $("#"+this.id+"-end").change($.proxy(function() {
            const val = $("#" + this.id + "-end").val().trim();
            if (!this.checkCoordinates(val))
                alert("Coordinate non valide");
            else
                this.tripWidget.module.setEndPoint(this.extractLatLng(val));
        }, this));
    ...
    extractLatLng: function (coordinates) {
        const v = coordinates.match(/^\((-?\d+(?:\.\d+)?),\s*(-?\d+(?:\.\d+)?)\)$/);
        return L.latLng(v[1], v[2]);
    }
...
otp.widgets.tripoptions.TimeSelector =
    ...
    doAfterLayout : function() {
        ...
        $("#"+this.id+'-locale').text(this.tripWidget.module.date);
        $("#"+this.id+'-locale-string').text(otp.config.locale.config.name);
    ...
    restorePlan : function(data) {
        ...
        if(data.queryParams.date) {
            ...
            // La versione originale non formattava correttamente la data
            this.tripWidget.module.date = moment(data.queryParams.date, otp.config.apiDateFormat).format(otp.config.locale.time.date_format);
            $("#"+this.id+'-locale').text(this.tripWidget.module.date);
        }
...
otp.widgets.tripoptions.SortedSelector =
    otp.Class(otp.widgets.tripoptions.TripOptionsWidgetControl, {

        id           :  null,
        //TRANSLATORS: label for checkbox
        label        : _tr("Sort_by_time"),
        ...
        doAfterLayout : function() {
            ...
            $("#"+this.id+"-sorted-input").change(function() {
                this_.tripWidget.module.sorted = this.checked;
            });
        },

        restorePlan : function(data) {
            var checked = data.queryParams.sorted === true || data.queryParams.sorted === 'true';
            this.tripWidget.module.sorted = checked;
            $("#"+this.id+"-sorted-input").prop("checked", checked);
        },

        isApplicableForMode : function(mode) {
            return otp.util.Itin.isCombined(mode);
        }
    });
...
otp.widgets.tripoptions.CombinedModeWidget =
    ...
    restorePlan : function(data) {
        /*var checked = data.queryParams.wheelchair === true || data.queryParams.wheelchair === 'true';
        this.tripWidget.module.wheelchair = checked;*/
        //$("#"+this.id+"-car-transit-input").prop("checked", checked);
    },
...
otp.widgets.tripoptions.ModeSelector =
    ...
    combinedModes: otp.config.multimodes,
    ...
    refreshModeControls : function() {
        ...
        if (mode === undefined) {
            mode = _.keys(this.combinedModes)[document.getElementById(this.id).selectedIndex - _.keys(this.modes).length]
        }
