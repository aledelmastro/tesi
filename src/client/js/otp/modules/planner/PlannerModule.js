...
otp.modules.planner.defaultQueryParams = {
    ...
    date                            : moment().format(otp.config.apiDateFormat),
    arriveBy                        : false,
    wheelchair                      : false,
    mode                            : "TRANSIT,WALK",
...
otp.modules.planner.PlannerModule =
    ...
    */
    ...
    planTrip : function(existingQueryParams, apiMethod) {
        ...
        var modes = []
        if (otp.config.modes[this.mode]) {
            modes.push(this.mode);
            this.multimodes = {}
        } else {
            modes = _.keys(this.multimode).filter(k => this.multimode[k])
            if (modes.length === 0)
                return;
        }
        ...
        for (i = 0; i < modes.length; i++) {
            // Così mantiene l'ultima delle combinate se non cambi voce nella tendina
            //this.mode = modes[i];
            var _mode = modes[i];
    ...
    highlightLeg : function(leg) {
        ...
        var polyline = new L.Polyline(otp.util.Geo.decodePolyline(leg.legGeometry.points));
        polyline.setStyle({ color : "yellow", weight: 16, opacity: 0.6 });
