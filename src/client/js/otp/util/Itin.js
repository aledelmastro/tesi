...
otp.util.Itin = {
    ...
    isCombined : function(mode) {
        var modeArr = mode.split(":");
        return modeArr[0] === 'COMBINED';
    },
    ...
    /** Se il gtfs contiene delle indicazioni sui colori di alcune strade,
    * OTP userà quelli per colorare le tratte
    @see getLegBackgroundColor */
    getModeColor : function(mode) {
        if(mode === "WALK") return '#00dcff';
        if(mode === "BICYCLE") return '#44f';
        if(mode === "SCOOTER") return '#88f';
        if(mode === "CAR") return '#316031';
